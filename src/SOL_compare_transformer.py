...
# ログファイルのパス
log_file_path = os.path.join(log_dir, 'SOL_TransformerCompetitor'+formatted_time+'.log')
...
class LogicTransformer(nn.Module):
    def __init__(self, XSize, YSize, d_model=64, nhead=8, num_layers=6):
        super().__init__()
        self.embedding = nn.Linear(XSize, d_model)  # ビット列を埋め込みに変換
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=d_model, nhead=nhead, batch_first=True)
#        self.encoder_layer = nn.TransformerEncoderLayer(d_model=d_model, nhead=nhead)
        self.transformer = nn.TransformerEncoder(self.encoder_layer, num_layers=num_layers)
        self.fc_out = nn.Linear(d_model, YSize)  # Yの各ビットを出力

    def forward(self, x):
        x = self.embedding(x)  # (batch, XSize) → (batch, d_model)
        x = self.transformer(x.unsqueeze(1))  # Transformer の入力形式に変換
#        return torch.sigmoid(self.fc_out(x.squeeze(1)))  # 出力を 0~1 に制限
        return self.fc_out(x.squeeze(1))  # 出力を 0~1 に制限
...
if __name__ == "__main__":
    ...
    # Hyperparameters
    model = LogicTransformer(XSize=32, YSize=16,d_model=_d_model,num_layers=_num_layers)  # YSize は論理式の数
